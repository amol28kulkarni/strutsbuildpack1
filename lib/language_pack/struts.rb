...
module LanguagePack
  class Struts < JavaWeb
    include LanguagePack::AutostagingHelpers

    DEFAULT_APP_CONTEXT = "/WEB-INF/struts.xml"
    ...
    def self.use?
      ...
      unless spring_files_found
        spring_files_found = (Dir.glob("webapps/ROOT/WEB-INF/classes/org/springframework").any? ||
            Dir.glob("webapps/ROOT/WEB-INF/lib/struts-core*.jar").any? ||
            Dir.glob("webapps/ROOT/WEB-INF/lib/xwork-core*.jar").any?) ||
			Dir.glob("webapps/ROOT/WEB-INF/lib/ognl*.jar").any?)
      end
      spring_files_found
    end
    ...
    def compile
      ...
      Dir.chdir(webapp_path) do
        configure_autostaging unless system_properties["struts.autoconfig"] == false
      end
    end

    def default_app_context
      if File.exist? (File.join(webapp_path, 'WEB-INF','struts.xml'))
        DEFAULT_APP_CONTEXT
      end
    end
